...
pygame.display.set_caption("Burger Dog: Survive!")
...
player_lives = PLAYER_LIVES_STARTING
...
while running:
    ...
    if burger_rect.y > WINDOW_HEIGHT:
        ...
        #reset player and put at bottom center of the screen
        #also reset boost level
        player_rect.centerx = WINDOW_WIDTH//2
        player_rect.bottom = WINDOW_HEIGHT
        boost_level = STARTING_BOOST_LEVEL

    #check to see if player and burger have collided
    if player_rect.colliderect(burger_rect):
        ...
        #if you have eatern a burger (or collided) you get an extra 25 in boost_level
        boost_level += 25
        if boost_level > STARTING_BOOST_LEVEL:
            boost_level = STARTING_BOOST_LEVEL

    #update HUD by re-rendering text
    #simply copy rendering from above
    points_text = font.render("Points: " + str(burger_points), True, ORANGE)
    score_text = font.render("Score: " + str(score), True, ORANGE)
    eaten_text = font.render("Burgers eaten: " + str(burgers_eaten), True, ORANGE)
    lives_text = font.render("Lives: " + str(player_lives), True, ORANGE)
    ...
    #check for game over
    if player_lives == 0:
        #only need to re-render what has chaged
        game_over_text = font.render("FINAL SCORE: " + str(score), True, ORANGE)
        #blit to surface game over text and continue text
    ...
    #draw a white line beneath the HUD
